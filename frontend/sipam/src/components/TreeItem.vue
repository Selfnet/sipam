<template>
  <div>
    <div :class="{bold: isFolder}">
      <fai @click="toggle" v-if="isFolder" :icon="isOpen ? 'minus-circle': 'plus-circle'" class="pointer"/>
      <cidr :cidrID="item.id" v-on:cidrClicked="toggle"></cidr>
    </div>
    <b-list-group-item class="nested" v-show="isOpen" v-if="isFolder">
      <tree-item
        class="item"
...
export default {
  ...
  created() {
    this.$store.subscribe((mutation, state) => {
      if (mutation.type === 'CIDR/SET_CIDR' && mutation.payload.parent === this.item.id && !this.childIDs.includes(mutation.payload.id)) {
        this.getSetChildren()
