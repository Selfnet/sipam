...
class LabelTest(TestCase):
    ...
    def test_add_label(self):
        ...
        newLabel = {"name": "TestLabel", "value": "TestValue"}

        serializer = LabelSerializer(data=newLabel)
        assert serializer.is_valid()

        request = factory.post(reverse("cidr-labels-list", kwargs={"cidr_pk": self.cidr.id}), newLabel)
    ...
    def test_update_label(self):
        ...
        updatedLabel = {"WithLabelKey": "UpdatedValue"}
        ...
        for token, rcode in [(self.token, 204), (self.rtoken, 403)]:
            request = factory.put(
                reverse("cidr-labels-detail", kwargs={"pk": "WithLabelKey", "cidr_pk": self.cidr.id}), updatedLabel
            )
            force_authenticate(request, token=token)
    ...
    def test_delete_label(self):
        ...
        request = factory.delete(
            reverse("cidr-labels-detail", kwargs={"pk": "DeleteLabelKey", "cidr_pk": self.cidr.id})
        )
        ...
        request = factory.delete(
            reverse("cidr-labels-detail", kwargs={"pk": "DeleteNonExistingLabelKey", "cidr_pk": self.cidr.id})
        )
        force_authenticate(request, user=self.admin, token=self.token)
